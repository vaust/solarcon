...
void *server_thread( void *arg )
{
    ...
    sprintf( bufout, "\n Mögliche Befehle: \n\n" );
    write( fdesc, bufout, strlen( bufout ) );
    sprintf( bufout, "\t GET TEMPS (alle Temperaturmesswerte)\n" );
    write( fdesc, bufout, strlen( bufout ) );
...
int parseGet( int fdesc, char *bufout )
{
    ...
    token = strtok( NULL, "\n\r " );
    if( strncasecmp( token, "TEMPS", 5 ) == 0 ) {
        sprintf( bufout, "ALL_Tau_MW     = %5.1f °C\n", toGradCelsius( ALL_Tau_MW ) );
        write( fdesc, bufout, strlen( bufout ) );
        ...
        sprintf( bufout, "kes_Tvl_SW_Sp1_f = %5.1f °C\n", kes_Tvl_SW_Sp1_f );
        write( fdesc, bufout, strlen( bufout ) );
        sprintf( bufout, "kes_Tvl_SW_Sp2_f = %5.1f °C\n", kes_Tvl_SW_Sp2_f );
        write( fdesc, bufout, strlen( bufout ) );
        ...
        sprintf( bufout, "ww_tww_sw       = %5.1f °C\n", toGradCelsius( ww_tww_sw ) );
        write( fdesc, bufout, strlen( bufout ) );
/*
        sprintf( bufout, "ww_HZG_Tvl_SW_f = %5.1f °C\n", ww_HZG_Tvl_SW_f );
        write( fdesc, bufout, strlen( bufout ) );
*/
    }
        ...
        sprintf( bufout, "KES_Tvl_Y    = %5d\n", KES_Tvl_Y );
        write( fdesc, bufout, strlen( bufout ) );
        sprintf( bufout, "HK_MV_Y      = %5d\n", HK_MV_Y );
        write( fdesc, bufout, strlen( bufout ) );
        sprintf( bufout, "FB_PRIM_MV_Y = %5d\n", FB_PRIM_MV_Y );
        write( fdesc, bufout, strlen( bufout ) );
        sprintf( bufout, "WW_HZG_MV_Y  = %5d\n", WW_HZG_MV_Y );
        write( fdesc, bufout, strlen( bufout ) );
        sprintf( bufout, "WW_HZG_PU_Y  = %5d\n", WW_HZG_PU_Y );
        write( fdesc, bufout, strlen( bufout ) );
    }
