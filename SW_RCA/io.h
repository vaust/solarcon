...
#define WW_PARTY		(pabIn_p->din.ww_party)		    /* Warmwasser, Partyschalter, DI, Taster 	*/								/* Partyschalter, Warmwasserbereitung, DI, 24V */
#define STOERUNG        (pabOut_p->dout.stoerung)
#define CONTROL_AKTIV   (pabOut_p->dout.control_aktiv)
...
typedef struct {
	...
	struct {
		...
		do_bitbyte_t res2			:1;		/* reserviert fuer Primärmischventil Fußbodenheizung                Slot 8, O4 */
   		do_bitbyte_t res3			:1;		/* reserviert fuer Primärmischventil Fußbodenheizung                Slot 8, O4 */
        do_bitbyte_t control_aktiv	:1;     /* Kontrolllampe Steuerung aktiv   */
        do_bitbyte_t stoerung		:1;     /* Kontrolllampe Sammelstörmeldung */
