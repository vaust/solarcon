...
#        self.SCHRIFT = ('Arial', 11)
    ...
    def drawLabels(self):
        ...
        self.mv_lbl = dict()
        for ao_name in signals.AO_NAMES:
            self.mv_lbl[ao_name] = tk.Label( self, width=self.L, bg=self.HFARBE, relief='sunken', font=self.SCHRIFT, text='---%' )
        ...
        self.main.create_window( 993,  855, window=self.mv_lbl["FB_PRIM_MV_Y"])
        self.main.create_window( 772,  855, window=self.mv_lbl["HK_MV_Y"])
        self.main.create_window( 750, 1071, window=self.mv_lbl["WW_HZG_VV_Y"])
        self.main.create_window( 538,  684, window=self.mv_lbl["WW_HZG_PU_Y"])
        ##
    ...
    def updateLabels(self, t_dict, pu_dict, mv_dict, di_dict, cnt_dict, av_dict):
        for name in signals.TEMP_NAMES:
            self.temp_lbl[name].config( text=str(t_dict[name])+' °C' )
        
        for name in signals.PU_NAMES:
            self.pu_lbl[name].config( text=str(pu_dict[name]) )

        for name in signals.AO_NAMES:
            self.mv_lbl[name].config( text=str(mv_dict[name])+' %' )

        self.cnt_lbl["WW_WZ_MW"].config( text=str(cnt_dict["WW_WZ_MW"]) )

        for name in signals.AV_NAMES:
            self.av_lbl[name].config( text=str(av_dict[name]) )

        for name in signals.DI_NAMES:
            self.di_lbl[name].config( text=str(di_dict[name]) )
