...
typedef struct param_hk_s {
    float tvl_steigung;     /**< Heizkoerperheizkreis Heizkurvensteigung                                  */
    float tvl_niveau;       /**< Heizkoerperheizkreis Heizkurvenparallelverschiebung                      */
    float tvl_absenk;       /**< Heizkoerperheizkreis Nachtabsenkung in Grad C                            */
    float tvl_min;          /**< Heizkoerperheizkreis Frostschutztemperatur in Grad C                     */
    float tvl_max;          /**< Heizkoerperheizkreis maximale Vorlauftemperatur in Grad C                */
    float frostschutz;      /**< Aussentemperatur ab der Heizkreis in Betrieb ist in Grad C               */
    float reg_kp;           /**< Heizkoerperheizkreis Verstaerkung PI-Regler in %/K                       */
    float reg_ki;           /**< Heizkoerperheizkreis Verstaerkung PI-Regler I-Anteil in %/(K x s)        */
    float reg_ap;           /**< Heizkoerperheizkreis Arbeitspunktoffset in %                             */
    float tr_sw;            /**< Heizkoerperheizkreis Raumtemperatur Sollwert                             */
} param_hk_t;

typedef struct param_fb_s {
    float tvl_steigung;     /**< Fussbodenheizkreis Heizkurvensteigung                                    */
    float tvl_niveau;       /**< Fussbodenheizkreis Heizkurven Parallelverschiebung                       */
    float tr_sw;            /**< Fussbodenheizkreis Raumtemperatursollwert in Grad C                      */
    float tvl_absenk;       /**< Fussbodenheizkreis Nachtabsenkung in Grad C                              */
    float tvl_min;          /**< Fussbodenheizkreis Frostschutztemperatur in Grad C                       */
    float tvl_max;          /**< Fussbodenheizkreis max. Vorlauftemperatur in Grad C                      */
    float reg_kp;           /**< Fussbodenheizkreis Verstaerkung PI-Regler in %/K                         */
    float reg_ki;           /**< Fussbodenheizkreis Verstaerkung PI-Regler I-Anteil in %/(K x s)          */
    float reg_ap;           /**< Fussbodenheizkreis Arbeitspunktoffset in %                               */
} param_fb_t;

typedef struct param_ww_s {
    float ww_tww_sw;           /**< Warmwasser-Temperatur-Sollwert in Grad C                              */
    float ww_tww_max;          /**< Warmwasser-Maximaltemperatur in Grad C                                */
    float wz_faktor;           /**< Waermezaehler auf Pumpenstellbefehl                                   */
    float ww_wz_max;           /**< Max. Warmwasserdurchfluss in l/min                                    */
    float ww_pu_reg_kp;        /**< WW-Temperatur-Regelung (Pumpe) Verstaerkung Kp                        */
    float ww_pu_reg_ki;        /**< WW-Temperatur-Regelung (Pumpe) Verstaerkung Ki in %/(K x s)           */
    float ww_pu_reg_ap;        /**< WW-Temperatur-Regelung (Pumpe) Arbeitspunktoffset in %                */
    float ww_mv_korr;          /**< WW Hzg-VL-Temperatur-Steurung (Mischer) Korrekturfaktor               */

    float ww_tww_tvl_faktor;   /**< Einfluss der WW-Temperatur auf die Heizungs-VL-Temp.                  */
    float ww_tz_sw;            /**< Temperatursollwert des Zirkulationswassers in Grad C                  */
} param_ww_t;
...
const parse_set_t param_Vorgaben[] = {
    ...
    { "HK_Tvl_Steigung",     &(param.hk.tvl_steigung),     "%f" },
    { "HK_Tvl_Niveau",       &(param.hk.tvl_niveau),       "%f" },
    { "HK_Tvl_Absenk",       &(param.hk.tvl_absenk),       "%f" },
    { "HK_Tvl_Min",          &(param.hk.tvl_min),          "%f" },
    { "HK_Tvl_Max",          &(param.hk.tvl_max),          "%f" },
    { "HK_Frostschutz",      &(param.hk.frostschutz),      "%f" },
    { "HK_REG_Kp",           &(param.hk.reg_kp),           "%f" },
    { "HK_REG_Ki",           &(param.hk.reg_ki),           "%f" },
    { "HK_REG_AP",           &(param.hk.reg_ap),           "%f" },
    { "HK_Tr_SW",            &(param.hk.tr_sw),            "%f" },
    { "FB_Tvl_Steigung",     &(param.fb.tvl_steigung),     "%f" },
    { "FB_Tvl_Niveau",       &(param.fb.tvl_niveau),       "%f" },
    { "FB_Tr_SW",            &(param.fb.tr_sw),            "%f" },
    { "FB_Tvl_Absenk",       &(param.fb.tvl_absenk),       "%f" },
    { "FB_Tvl_Min",          &(param.fb.tvl_min),          "%f" },
    { "FB_Tvl_Max",          &(param.fb.tvl_max),          "%f" },
    { "FB_REG_Kp",           &(param.fb.reg_kp),           "%f" },
    { "FB_REG_Ki",           &(param.fb.reg_ki),           "%f" },
    { "FB_REG_AP",           &(param.fb.reg_ap),           "%f" },
    { "WW_Tww_SW",           &(param.ww.tww_sw),           "%f" },
    { "WW_Tww_Max",          &(param.ww.tww_max),          "%f" },
    { "WZ_Faktor",           &(param.wz.faktor),           "%f" },
    { "WZ_Max",              &(param.ww.wz_max),           "%f" },
    { "WW_PU_REG_Kp",        &(param.ww.pu_reg_kp),        "%f" },
    { "WW_PU_REG_Ki",        &(param.ww.pu_reg_ki),        "%f" },
    { "WW_PU_REG_AP",        &(param.ww.pu_reg_ap),        "%f" },
